...
Documents are analogous to Django models, but for OpenSearch instead of a database. For the simplest cases, you can
...
Notice in the example above that ``author`` is overridden to use `OpenSearch object type`_, and ``word_count`` is an extra field not
defined by the ``Post`` model.

.. _`OpenSearch object type`: https://www.elastic.co/guide/en/elasticsearch/reference/1.7/mapping-object-type.html
...
When re-indexing a mapping, the process is as follows:
    ...
    5. :meth:`seeker.mapping.ModelIndex.get_id` and :meth:`seeker.mapping.ModelIndex.serialize` are called to generate the ID and data sent to OpenSearch for each object.
